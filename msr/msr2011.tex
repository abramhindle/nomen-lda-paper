...
First of all, we automate the process of naming the topics. Secondly, we label topics with project-independent terms, in order to permit cross-project comparison.
...
As of version 7.500, released April 2007, the project has over $940,000$ lines of C source code\footnote{generated using David A. Wheeler's \emph{SLOCCount}, http://dwheeler.com/sloccount.}. 
...
with the relevant label and sometimes they used finer-grained
...
For each quality we assessed whether semi-unsupervised labels matched the manual annotations. %To be clear, the manual annotations were not used to train the labelling process.
...
class/label to assign a label to a topic. In our approach, we use the top-level NFRs in the ISO 9126 standard~\cite{iso9126} for our classes, but other taxonomies are also applicable.%In order to validate how effective these word-bag approaches to topic labelling would be we created a validation data set. 
...
\label{fig:maxdb-timeline}
	...
	Grid cell intensity (saturation) is mapped to label frequency
        ...
        relate a project milestone (a \emph{key event}) to our topic windows. 
% \todo[inline]{better discussion}
}
\label{fig:timelines}
\end{figure*}

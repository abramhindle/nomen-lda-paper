...
``\emph{event handling}'' by a human who understands the domain well,
despite the fact that label does not appear in the word list itself.  
...
In this paper, we introduce \emph{labelled topic extraction}, an approach to topic labelling that is both automatic and creates labels that are project independent.
...
For instance a manager could confirm if their developers were
focusing on \emph{usability} by looking for
...
\item evaluates three kinds of automatic topic labelling methods:
  semi-unsupervised labelling (word-lists) of topics, supervised labelling of
...
Cleland-Huang and her colleagues have research on mining requirements documents for non-functional requirements (NFR) (quality requirements)~\cite{Cleland-Huang2006}. 
Their approach is similar to ours, as they mined keywords from NFR catalogues.
...
Another difference is that our data was far less structured than the requirements documents used in their study.
...
They only used WordNet for word roots as they felt the synonyms would be non-specific and cause errors, problems we also encountered. 
...
With ConcernLines, Treude et al.~\cite{treude09cl} show tag occurrence using colour intensity. 
They mine
...
They do not use external domain-specific information as we do and we
...
This research extends Hindle et al.~\cite{Hindle09ICSM} with labelled
...
and MaxDB 7.500, we (the first two authors) annotated each extracted topic in each period with
...
with the relevant label and sometimes we used finer-grained
...
%, an English-language ``lexical database'' that contains semantic relations between words, including common related forms (similar to word stemming), meronymy and synonymy. 
...
For each quality we assessed whether semi-unsupervised labels matched the manual annotations. %To be clear, the manual annotations were not used to train the labeling process.
...
\item \emph{Do projects differ in their relative interest in NFRs?} A
  project manager, especially a systems-manager, would be interested in knowing whether a particular NFR, such as \emph{reliability}, was more important for one project than another. This could be to confirm the initial design goals, or to track the progress on that quarter's objectives.  
\end{enumerate}
...
We did not attempt to match our results to any particular model or
theory.
Also we did not have enough manpower to assess inter-rater reliability.
...
To improve on this, we leveraged software engineering standards, specifically the ISO quality taxonomy, to produce a method of partially-automated (supervised) and fully-automated (semi-unsupervised) topic labelling.

...
These topics are given labels from to a generalizable
...
range from project dashboards to annotating software artifacts with
NFR-related tags such as revisions and bug reports. 
Project dashboard are typically employed by managers and are used to
provide quick summaries of a the efforts put into a software project,
labelled topic extraction would allow managers to track efforts
related to topics, specifically NFRs.
These techniques also allow the annotation of bug
reports and other software artifacts with the NFRs, such as
portability that they relate to, this would enable querying of bug
reports and artifacts by relevant NFRs. 
For instance one could look
for portability relevant revisions and bug reports.
...
Next, we describe how we generated our data (Section \ref{sec:wordlist}). For unsupervised classification (Section \ref{sec:unsuplabelling}), we begin by creating word-lists to signifiy when a topic matches an NFR label. We then apply our classifier and analyze the results. %and how we created the word-lists used in unsupervised classification. 
...
To evaluate both unsupervised and supervised classification, we created a validation set of manually labelled topics. For MySQL 3.23 and MaxDB 7.500, we annotated each extracted topic in each period with the same NFR labels as described in the preceding paragraph. 
...
abbreviated \emph{ROC}, and the F-measure.  F-measure is the harmonic mean of precision and recall.
...
In order to automatically label each topic with an NFR from ISO9126, we associate each NFR with a list
...
This analysis allows to address the questions raised in Section \ref{sec:analysis}:

...
Topic modeling is a machine learning technique which creates multinomial distributions over words from a text corpus. Topic models are useful in software engineering because they can identify co-occuring words from text corpora such as source code, commit comments, email, and others. For example, a topic extracted from a set of commit logs might be\emph{ ``listener change remove add fire''}. A topic model is a summary of the key concepts in the corpus. Among other uses, it can quickly give developers an overview of where activity is occurring and gives managers or maintainers a sense of project history. A major challenge is to accurately identify the summary label for a topic (in the previous case, perhaps \emph{``event-handling''}). 
Current approaches are manual: they rely on human expertise to recognize the appropriate label. Furthermore, to date the results of topic modeling are project specific, and not generalizable. This paper presents an approach, labelled topic extraction, which addresses these two problems.

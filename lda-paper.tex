...
\newcommand{\hlc}[1]{{\color{changecolor3} #1 }}
\newcommand{\hld}[1]{{\color{changecolor4} #1 }}
...
\hla{ The questions that we wish to address deal with the past and
  topics of development that occurred in the past.  What happened in
...
\hla{
Usually topic analysis is applied to the entire history of a project~\cite{1321709,lukins2008,10.1109/ICPC.2007.13},
}
...
example we have titled each topic with a single word drawn from its
word distribution.  
\hdb{
...
\hdb{ Visualization is an integral part of our topics analysis which
  ...
  topics were independant per period, what the repeating trends were,
...
attempts to show a summary of trends seperated from single topics. This view shows how a trend persists across time, which aids time-wise analysis of trends.
}
...
\hdb{ The \emph{trend-histogram} (Figure \ref{fig:histogram}) attempts
...
long. \hdb{ The \emph{trend-histogram} summarizes the distribution of
trends ordered by size. }

\hdb{ All of these visualization combine to enable an analysis of

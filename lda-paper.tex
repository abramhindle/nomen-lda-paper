...
message, to be related to multiple topics. This maps well to
commits which often have more than one purpose.  A topic represents
both a word distribution and a group of commit log comments that are
related to each other by their content.  In this case a topic is a set
of tokens.
...
example we have titled each topic with a single word drawn from
itself.
...
locally relevant and thus a windowed analysis can allow for both
different kinds of global and local analysis than global topic
analysis would.

Our contributions include:
\begin{itemize}
\item Demonstrating that windowed topic analysis most topics are local
  in scope, thus would be missed by a global topic analysis.
\item Multiple visualizations of topics and trends over time to aid
  communication and analysis of topics and trends.
\item An exploratory case study using these techniques on several open
...
top 10 matching words. We then apply the transitive closure on similar
topics to this topic similarity matrix; this is similar to modelling
topics as nodes and similarity as arcs, then fill flooding along the
...
%  \includegraphics[width=1.0\textwidth]{fixed-time-smear-plot-scaled}
  ...
  \caption{Compact-trend-view shows topics per month for MaxDB
    ...
    joined and embedded in box representing the topics. The gap in the

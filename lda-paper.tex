...
%   Currently it is difficult for stakeholders to
...
%   see the evolving stream of topics of development occuring
%   over time.
...
features.  We propose to help stakeholders understand the topics of
...
purposes, thus we hope that multipurposes manifest themselves as
...
distribution that is unique and independant within the set of
...
extracted from SCS, Linstead et al proposed an author-sourcecode model
...
This is where concepts are automatically extracted from source code or
documents related to the source code.  Concept location is how high
level concepts relate to low level entities like source code, for
instance when fixing a bug how do the concepts in the bug report map
back to the source code?  Semantic Clustering has also been used for
similar reasons~\cite{1698774,1566153} as it is similar to LSI.
...
commit log message of each commit, count the occurence of each word in

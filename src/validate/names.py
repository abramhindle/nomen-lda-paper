...
class Taxonomy():
    ...
    usability = lowerlist( usability_mccall + usability_user + usability_syn + usability_hyper + usability_deriv + usability_meronym ) #+ usability_spell 
    ...
    functionality = lowerlist( functionality_mccall + functionality_user + functionality_syn + functionality_hyper + functionality_deriv + functionality_meronym )#+ _spell
    ...
    reliability = lowerlist( reliability_user + reliability_mccall + reliability_syn + reliability_hyper + reliability_deriv + reliability_meronym ) #+ _spell
    ...
    maintainability = lowerlist( maintainability_user + maintainability_mccall + maintainability_syn + maintainability_hyper + maintainability_deriv + maintainability_meronym ) #+ _spell
    ...
    portability = lowerlist( portability_mccall + portability_user + portability_syn + portability_hyper + portability_deriv + portability_meronym ) #+ _spell
    ...
    efficiency =  lowerlist( efficiency_user + efficiency_syn + efficiency_hyper + efficiency_deriv + efficiency_meronym ) #+ _spell
    ...
    def find_signifier(self, key):
        key = key.lower()
        ss = [x.lower() for x in signifier_dict.keys()]
        if key in ss:
            return key
        for (s,d) in signifier_dict.items():
            if key in d:
                return s
        raise NameError(key+" not found in signifiers")

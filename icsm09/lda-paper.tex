...
\begin{abstract}
  ...
  have been implementing new features.  
%  have been implementing a set of new features.  
%
  Tools such as Latent Dirichlet Allocation (LDA) and Latent Semantic
%  Indexing (LSI) can be used to extract a set of independent topics
  Indexing (LSI) can be used to extract a set of independent topics
  from a corpus of commit-log comments.  Previous work in the area has
  created a single set of topics by analyzing comments from the entire
  lifetime of the project.  In this paper, we propose windowing the
  topic analysis to give a more nuanced view of the system's
  evolution. By using a defined time-window of, for example, one
  month, we can track which topics come and go over time, and which
  ones recur.
%
%
%   Tools such as Latent
%   Dirichlet Allocation (LDA) and Latent Semantic Indexing (LSI) can be
%   used to extract a set of independent topics from a corpus of commit
%   log comments.  Previous work in the area has created a single set of
...
%   topics come and go over time, and which ones recur.
%  
  We propose
  ...
  applied to a project's lifetime because many topics are quite
  local.
...
%   for example, a manager might wonder what unexpected tasks occupied
%   their team's attention during a period when they were supposed to
%   have been implementing a set of new features.  Tools such as Latent
%   Dirichlet Allocation (LDA) and  Latent Semantic Indexing (LSI)
%     can be used to analyze commit log  comments over the entire
%   lifetime of a project. Previous work on developer topic analysis has
%   leveraged these tools to associate commit log comments with
%   independent topics extracted from these commit log comments.  In
%   this paper, we use LDA to analyze periods, such as months, within a
%   project's lifetime to create a time-windowed model of changing
%   development topics.  We propose visualizations of this model that
%   allows us to explore the evolving stream of topics of development
%   occurring over time.  We demonstrate that windowed topic analysis
%   offers advantages over topic analysis applied to a project's
%   lifetime because many topics are quite local.
...
and over between releases?  How long do different topics persist?  Are
...
% whereas this paper suggests that most topics are local. For example,  
...
literature~\cite{lukins2008-short,10.1109/MSR.2007.20-short} to %,NIPS2007637,1321709}.
extract topics from documents such as methods, bug
...
%LDA
%does not name these clusters but the clusters consist of
words whose
...
using LDA~\cite{10.1109/MSR.2007.20-short}. This %,NIPS2007637,1321709}. 
model associates authors and topics via their associated documents.
...
%These queries would indicate the proportion of
...
we would get the word distributions of each commit message, 
\begin{comment}
pass them to
...
\end{comment}
% (please preserve the italics you put in ... I had to edit this in plain text so they got lost)
...
%  We chose the top 10 most frequent words because sometimes a couple
...
%   We decided [did we?] to treat these as stop words, since they occur
...
%they are related to version control usage.  
...
% mentions of BitKeeper. 
%\emph{RENAME} also only appeared once in the
...
the top 10 words was usually accurate.

%SHORTEN could shorten 1 line
...
% For each commit log comment, we count the occurrence of each word in
% the message, remove stop words, and then produce word distributions
% for each message, which are normalized (divided by) by the size
% of the message.
% After all messages are processed the distributions are
% extended to include all words from all of the distributions.
% 
%
...
then normalizing the distribution by the size of the message, in tokens.
% (dividing word counts by the number of tokens).  
After
all messages are processed the distributions are extended to include
all words from all of the distributions.

%, and then produce word distributions
%for each message.  
%These distributions are then normalized by the size
%of the message, each count was divided by the total number of
%tokens. 
...
about top 10 comparisons and it allowed for some common words to exist
%but not cause topics to match so closely.  
...
%       1. [ ] need the visualizer
  Visualization is an integral part of our topic analysis which
  allows us to quickly explore the topics and trends of a project.
  Visualizing the results %is desirable as it 
...
each trend gets its own distinct y-value, while the x-axis is time;
...
%the color of a unique topic is grey. 
Color similarity is not meaningful.  
%The compact-trend-view is
...
topic ($90\%$ to $99\%$ of the total topics), otherwise the tail is
...
We examined PostgreSQL's history from 1996 to 2004, which includes
...
optimizer is an important component of a DBMS, but as we have shown
...
analysis such as our windowed topic analysis, which used a smaller window of
messages.
...
development. If implemented interactively, a user could easily zoom in
...
We wish to extend this study by further exploring parameter
 ...
 of topics.
%  We want to explore parameters more across multiple projects,
...
standards.
%\newline\noindent{\small\textbf{Acknowledgements:} We'd like to thank Tudor Girba and Neil Ernst.}
%\newline\noindent{\small\textbf{Acknowledgements:} We thank Tudor Girba for his comments.}
\newline\noindent{\small\textbf{Acknowledgements:} We would like to thank Tudor Girba and Neil Ernst for their feedback and ideas.}
%\newline\noindent{\small\textbf{Acknowledgements:} We thank Tudor Girba and Neil Ernst.}
%We'd like to thank Tudor Girba and Neil Ernst.}

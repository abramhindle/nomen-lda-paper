...
When a developer addresses a topic of development, such as a requirement, maintenance, bug fixing, tests or other software engineering tasks, they often produce or modify multiple development artifacts.
%
...
As well much of the current research stops short of interpreting the purpose of the topic. That is, topics extracted are usually manually named and labelled by readers.
% 
...
Often developers interact with abstractions of these artifacts, rather than the artifact itself. 
...
The topic of development is relevant because it shows anyone inspecting these topics what were the distinct issues or topics facing development at a certain time.
...
Thus our method of generalizing the data mining of these software artifacts is use the domain knowledge of software engineering, specifically NFRs. 
...
\todo[inline]{this table should have mysql}
\begin{table}
	\centering
...
\centering
\subfigure[MySQL]{
\includegraphics[width=0.4\textwidth]{mysql-supervised}
\label{fig:subfig1}
}
\subfigure[MaxDB]{
\includegraphics[width=0.4\textwidth]{maxdb-supervised}
\label{fig:subfig2}
}
...
\centering
\subfigure[MySQL]{
\includegraphics[width=0.4\textwidth]{multi-label-results-mysql}
\label{fig:subfig3}
}
\subfigure[MaxDB]{
\includegraphics[width=0.4\textwidth]{multi-label-results-maxdb}
\label{fig:subfig4}
}
\label{fig:mulan}
...
\begin{figure*}%[h]
  ...
  \includegraphics[width=.97\textwidth]{mysql-timeline}
  \caption{MySQL 3.23:NFRs over time. Topic frequency is mapped to alpha (transparency) value.}
  \label{fig:mysql}
\end{figure*}

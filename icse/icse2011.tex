...
While unsupervised techniques (LSI and LDA are both unsupervised) are appealing in their lack of human intervention, and thus lower effort, supervised learners have the advantage of domain knowledge (training sets) which typically means improved results. 
...
%XXX 
\textbf{Timeline of key events} -- Figs. \ref{} show key events for each of our projects.  % where's the figure?
Several external events (external to `normal' development activity, that is) affected our projects. 
For example, both projects show a high number of NFRs recognized at the first period of analysis. 
This is due to our window choice: we deliberately targeted our analysis to 
when MySQL 3.23 was first announced %Major update \& End-of life
and when MaxDB 7.500 was first announced.

Note that MySQL's release process was haphazard at this point: they would release a patch version whenever the need arose. 
By version 5.1, the company moved to a milestone release, with new releases targeted for every six months.

...
\textbf{Understanding the figures} -- For example, in Fig. \ref{fig:mysql-timeline}, we see that the NFRs \emph{functionality}, \emph{portability} and \emph{maintainability} contain more labeled topics, since these NFRs have been more intensely shaded. % boxes. not sure boxes is important..
%We note that the gap in the MaxDB data is due to an interruption in the source data, having to do with the infrastructure for releasing developer artifacts, rather than specific occurrences in the project.
% I grepped query1.mrs2 http://softwareprocess.es/y/y/maxdb-76.tar.gz
% and                   http://softwareprocess.es/y/maxdb-7500.tar.gz
The gap in the MaxDB is due to the fact that other versions of MaxDB exist on different repositories and during this period from February 2005 to Jun 2005 the focus of development shifted to MaxDB 7.6. 
Immediately after this period of inactivity, there was a large deluge of commits added to the MaxDB 7.500 repositories.

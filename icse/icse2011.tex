...
% the problem
 Unfortunately the output of these tools is often difficult to distinguish and interpret as they are often so abstract, for instance topic modelling produces word-lists that lack semantic interpretation.
% the solution 
 Thus to have a meaningful discussion about the topics of software development, we must be able to interpret these topics and devise appropriate labels for extracted topics, yet current software engineering topic analysis and concept analysis research relies on manual naming and labelling of topics
% the labelling research
Current topic labelling approaches neither use domain-specific knowledge to improve results, nor contextualize those results for developers.   
% a sub problem
...
% what we do
This paper implements \emph{labelled topic extraction}, in which topics are extracted from commit comments and given labels relating to a cross-project taxonomy. 
% data mining generalizable
To ensure we have a generalizable project agnostic approach we label topics by non-functional requirements related to software quality, since there is some shared belief that these qualities apply broadly across many software systems and their development artifacts.
...
\keywords{Topics, LDA, NFR}

\newcommand{\shrink}{}

\section{Introduction}

Topics in software engineering practice refer to specific `threads' between project stakeholders. These topics can include particular requirements, maintenance requests, bug fixes or other software engineering tasks. %NEIL: define topic specifically as akin to email 'thread'. It sounded too vague. Also changed from 'developers' to 'stakeholders' because developers sounds too coding oriented. Our work could apply to testers, users, etc.
...
%
Topics can be in several potential states: they may not be immediately resolved, they may re-occur, yet other topics may be only dealt with briefly, such as a  bug report that is closed.
...
The goal of concept location and topic analysis research is to recover these topics and their related software artifacts.
...
These artifacts are abstracted by their purpose or topic rather than their concrete representation.
...
Other such abstractions include project dashboards~\cite{kersten2005mylar}, tags~\cite{treude2010}, and complexity metrics \cite{mccabe1976complexity}. 
...
\todo[inline]{Update this, rewrite it to handle the new situation but be sure to keep the maintainability and portability}

\begin{figure*}[ht]
\centering
\subfigure[MaxDB 7.500]{
\includegraphics[width=\textwidth]{figures/maxdb-timeline-all}
\label{fig:mysql-timeline}
}
\subfigure[MySQL 3.23]{
\includegraphics[width=\textwidth]{figures/mysql-timeline-all}
\label{fig:maxdb-timeline}
}
\label{fig:timelines}
\caption[]{NFRs over time. Numbers represent topics labelled with that quality. Box intensity (saturation) is mapped to label frequency relative to the largest label value.
\todo[inline]{better discussion}
}
\end{figure*}
...
\todo[inline]{More needs to be done here! More content!}
\todo[inline]{Use the old diagrams to come up with interesting comparisons}
...
%\todo[inline]{We need to summarize what we did}
%\todo[inline]{We need to show our contribution}

...
\usepackage[caption=false]{subfig}
 \usepackage[colorinlistoftodos,backgroundcolor=yellow,disable]{todonotes} %disable
 \usepackage{color}
...
For example, in Fig. \ref{fig:mysql-timeline}, we see that the NFRs \emph{functionality}, \emph{portability} and \emph{maintainability} contain more labeled topics, since these NFRs have more intensely shaded boxes. We note that the gap in the MaxDB data is due to an interruption in the source data, having to do with the infrastructure for releasing data, rather than specific occurrences in the project.
\todo[inline]{can I pin this down more ?}

\todo[inline]{try to pin down these reasons below}%  MySQL 3.23 was put on maintenance.
% 

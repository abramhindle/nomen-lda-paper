...
%This kind of   approach typically makes no use of domain-specific knowledge to   improve the results or put them into a context that a developer can   relate to.  
...
%In this paper, we investigate what   topics the latent topic extractors produce, and examine whether   such topics can be generalized across software products.  
%We focus   on non-functional requirements related to software quality as a   potential generalization, since there is some shared belief that   these qualities apply broadly across many software systems and their   development artifacts.  
%In our approach, we name topics based on a   static ontology of non-functional requirements and software   qualities; in essence, this adds context to general-purpose   ontologies such as WordNet.  
%In this paper, we discuss how we use   this software development domain knowledge to better annotate topics   of development that are extracted from the change log messages in   configuration management systems such as CVS. 
% Since our taxonomy is   global in scope, our results allow us to compare the relative   importance of particular qualities across different projects. 
%Within the topics extracted from MySQL and MaxDB we found that   non-functional requirements were common and often were trends across   time. 
%Our automated techniques can label topics without much computational overhead.


\begin{comment}
 Within the field of mining software repositories, 
to have meaningful discussions about how software development is progressing, we must be able to devise appropriate labels for work topics. We can then categorize development artifacts as belonging to one or more of these topics. 
% Other approaches
In order to do this, approaches such as topic modeling and concept location rely on the automated application of machine learning algorithms to development artifacts. 
%limitations
...
%In our approach, we name topics based on a static ontology of non-functional requirements and software qualities; in essence, this adds context to general-purpose ontologies such as WordNet.  
%elaborate
%We discuss how we use this software development domain knowledge to better annotate topics of development that are extracted from the change log messages in configuration management systems such as CVS. 
We evaluated our approach with an experimental study on two large-scale database projects, MySQL and MaxDB. We extracted topics from their version control commit logs comments. Our results were generalizable across the two projects, showing that non-functional requirements were commonly discussed, and we identified topic trends over time. 	
%Our automated techniques can label topics with little computational overhead.  
%sum up
% Since our taxonomy is global in scope, our results allow us to compare the relative importance of particular qualities across different projects.
...
% area under the ROC 
% check
\emph{Area of ROC Curve} is the area under the Receiver Operating Characteristic (\emph{ROC}) curve, sometimes referred to as \emph{AUC}. ROC values reflect a score, similar to school letter-grades (A is 0.9, C is 0.6), reflecting how well a particular learner performed for the given data. A ROC result of 0.5 would be equivalent to a random learner (randomly classifying data). ROC maps to the more familiar concepts of precision/sensitivity and recall/specificity: it plots the true positive rate (sensitivity) versus the false positive rate (1 - specificity). A perfect learner has a ROC value of 1.0, reflecting perfect recall and precision.
...
monthly windows. This size of period is smaller than the time between minor releases but large enough for there to be sufficient commits to analyze. We applied Blei's LDA implementation~\cite{Blei2003} against the word distributions of these commits, and generated lists of topics per period. 
...
% NOTE I HAD TO CHANGE THE O HERE IN THE BIBTEX

...
In this paper we use LDA to identify topics. The input to the algorithm is a set of documents, with contents treated as a bag of words, and a parameter to set the number of topics to generate. The algorithm returns a word-document matrix relating the occurrence frequency of a 
...
into 30-day non-overlapping windows. Each window is subject to LDA and then we analyze and visualize the results. %Figure \ref{fig:commits}
